...
class SupabaseDatabase {
  ...
  async addProduct(product: Omit<Product, "id" | "createdAt" | "updatedAt" | "category">): Promise<Product | null> {
    try {
      const { data, error } = await supabase
        .from("products")
        .insert({
  ...
  async addCustomer(customer: Omit<Customer, "id" | "createdAt" | "updatedAt">): Promise<Customer | null> {
    try {
      const { data, error } = await supabase
        .from("customers")
        .insert({
  ...
  async addSale(sale: Omit<Sale, "id" | "createdAt" | "updatedAt" | "customer" | "items">): Promise<Sale | null> {
    try {
      const { data, error } = await supabase
        .from("sales")
        .insert({
  ...
  async addSaleItem(saleItem: Omit<SaleItem, "id" | "createdAt" | "product">): Promise<SaleItem | null> {
    try {
      const { data, error } = await supabase
        .from("sale_items")
        .insert({
  ...
  async addInvoice(invoice: Omit<Invoice, "id" | "createdAt" | "updatedAt" | "sale" | "customer">): Promise<Invoice | null> {
    try {
      const { data, error } = await supabase
        .from("invoices")
        .insert({
...
// تهيئة البيانات الافتراضية عند تحميل الملف
